...
```
Answer:the africa is most density continent and the south america is the most continent in plastic waste per cap
...
Answer here:because here we represent a set of data (continents),not just color control,as the colers here belong to the continents,and also the filling is related to the continents but the Alpha it's talking about all Graphic (gemo_density)
...
ggplot(data = plastic_waste, 
  ...
  geom_violin()
```
Answer:I see that the boxpolt is clearer and more precise than the violin plots, and also in the boxpolt the points are much clearer and come more regularly. 
...
ggplot(data = plastic_waste, 
  ...
  geom_point()
```
Answer5:The graph shows the relationship between plastic waste per capita and mismanaged plastic waste per capita using a scatter plot. The ratio mismanaged plastic and plastic waste relationship is almost the same,amd relationsip is positve
...
ggplot(data = plastic_waste, 
  ...
  geom_point()
```
Answer6:Countries became more pronounced with the addition and division of colors,example Europe(positive) relationship , Africa is the most (mismanaged_plastic_waste per cap)
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
```
The graph shows the distribution of plastic waste per capita and costal population proportion for each continent. The linear association is almost similar but there is more outlier in costal population.
